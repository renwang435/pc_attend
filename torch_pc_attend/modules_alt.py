...
class retina(object):
    ...
    """
    def __init__(self, num_points_per_pc, num_points_per_sample, num_samples, s, std, use_gpu):
    ...
    def foveate(self, x, l, d):
        ...
        for i in range(self.num_samples):
            phi.append(self.extract_box(x, l, d))
            d = torch.clamp(2 * d, 0.000, 1.001)
    ...
    def extract_box(self, imgs, l, d):
        ...
        Args
        ----
        - x: a 4D Tensor of shape (B, D, P). The minibatch
          of images.
        ...
        """
        B, C, D, P = imgs.shape
        imgs = imgs.view(-1, 3, self.num_points_per_pc)
        ...
        for i in range(B):
            ...
            if (im.numel() < self.num_points_per_sample * 3):
                num_missing = int(self.num_points_per_sample - im.numel() // 3)

                # If there are no points, fill with zeros
                # If there are some points, randomly sample from those points to get self.num_points
                if (num_missing == self.num_points_per_sample):
                ...
                idx = perm[:self.num_points_per_sample]
                im = im[:, idx]

            zooms.append(im.view((1, 3, self.num_points_per_sample)))
...
class glimpse_network(nn.Module):
    ...
    - g_t: a 2D tensor of shape (B, hidden_size). The glimpse
      ...
      current timestep `t`.
    """
    def __init__(self, h_g, h_l, num_points_per_pc, num_points_per_sample,
                 box_size, num_samples, s, c, std, use_gpu):
        super(glimpse_network, self).__init__()
        self.retina = retina(num_points_per_pc, num_points_per_sample,
                             num_samples, s, std, use_gpu)
        ...
        # box size
        D_in = 1
        self.fc3 = nn.Linear(D_in, h_l)

        self.fc4 = nn.Linear(h_g, h_g + h_l)
        self.fc5 = nn.Linear(h_l, h_g + h_l)
        self.fc6 = nn.Linear(h_l, h_g + h_l)

    def forward(self, x, l_t_prev, d_t_prev):
        # generate glimpse phi from image x
        phi = self.retina.foveate(x, l_t_prev, d_t_prev)
        phi = phi.view(-1, self.retina.num_points_per_sample, 3)

        # flatten location vector
        l_t_prev = l_t_prev.view(l_t_prev.size(0), -1)

        # flatten box size
        d_t_prev = d_t_prev.view(d_t_prev.size(0), -1)

        # feed phi, l and d to respective fc layers
        phi_out = F.relu(self.DTanh1(phi))
        l_out = F.relu(self.fc2(l_t_prev))
        d_out = F.relu(self.fc3(d_t_prev))

        what = self.fc4(phi_out)
        where = self.fc5(l_out)
        how = self.fc6(d_out)

        # feed to fc layer
        g_t = F.relu(what + where + how)
...
class box_network(nn.Module):
    """
    Uses the internal state `h_t` of the core network to
    produce the location coordinates `d_t` for the next
    time step.

    Concretely, feeds the hidden state `h_t` through a fc
    layer followed by a tanh to clamp the output beween
    [-1, 1]. This produces a 1D vector of means used to
    parametrize a one-component Gaussian with a fixed
    variance from which the location coordinates `l_t`
    for the next time step are sampled.

    Hence, the box size `d_t` is chosen stochastically
    from a distribution conditioned on an affine
    transformation of the hidden state vector `h_t`.
    ...
    """
    def __init__(self, input_size, output_size, std):
        super(box_network, self).__init__()
        self.std = std
        self.fc = nn.Linear(input_size, output_size)
    ...
    def forward(self, h_t):
        ...
        # bound between [-1, 1]
        # l_t = torch.tanh(l_t)
        d_t = torch.clamp(d_t, 0.300, 0.900)
